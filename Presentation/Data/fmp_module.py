...
def compute_X_dict(song_dict, song_selected, version='STFT', details=True, m_type=0):
    ...
    for s in song_selected:
        ...
        fn_wav = song_dict[s][0]
        fn_ann = song_dict[s][1]
        N = 2048
        H = 1024
        if version == 'STFT':
            X, Fs_X, x, Fs, x_dur = \
...
def get_chromagrams(song_selected, song_dict, Fs_X_dict_STFT, X_dict_STFT, Fs_X_dict_CQT, X_dict_CQT, Fs_X_dict_IIR, X_dict_IIR, cmap='gray_r', xlim=[0, 12]):
    ...
    for s in song_selected:
        ...
        libfmp.b.plot_chromagram(X_dict_STFT[s], Fs=Fs_X_dict_STFT[s], ax=[ax[0]],  
                             chroma_yticks=[0, 4, 7, 11], clim=[0, 1], cmap=cmap,
                             title=title, ylabel='Croma', xlabel='Tempo (segundos)', colorbar=True, xlim=xlim)
...
def get_chromagram(song_selected, song_dict, Fs_X_dict, X_dict, cmap='gray_r', chroma_type=0, xlim=[0, 12], figsize=(3.5, 2.5)):
    ...
    for s in song_selected:
        fig, ax = plt.subplots(1, 1, gridspec_kw={'width_ratios': [1], 
                                                  'height_ratios': [2]}, figsize=figsize)
...
def get_song_dict(audio_list, label_list):
    '''Creates a song dict from the input parameters'''
    n = len(audio_list)
    return {i: [audio_list[i], label_list[i]] for i in range(n)}
...
def txt_to_csv(input_file_path, output_file_path):
    # Read the TXT content from the file
    with open(input_file_path, 'r') as file:
        txt_content = file.read()

    # Convert the txt content to a csv format as requested
    csv_lines = ['"Start";"End";"Label"']  # Start with the header
    for line in txt_content.split('\n'):
        parts = line.split('\t')
        if len(parts) == 3:  # Ensure line has exactly 3 parts
            start, end, label = parts
            csv_line = f'{start};{end};"{label}"'  # Format each line
            csv_lines.append(csv_line)

    # Join the lines back into a single string to form the CSV content
    csv_content = '\n'.join(csv_lines)

    # Save the CSV content to the output file
    with open(output_file_path, 'w') as file:
        file.write(csv_content)

def txt_to_csv_with_modifications(input_file_path, output_file_path):
    # Read the TXT content from the file
    with open(input_file_path, 'r') as file:
        lines = file.readlines()

    # Initialize an empty list for the modified lines
    modified_lines = []

    # Process lines to adjust the first column values
    for i in range(len(lines)):
        parts = lines[i].strip().split('\t')
        if i == 0:
            # For the first row, set the first column to 0.000000
            parts[0] = '0.000000'
        else:
            # For subsequent rows, set the first column to the second column value of the previous row
            previous_parts = lines[i-1].strip().split('\t')
            parts[0] = previous_parts[1]

        modified_lines.append('\t'.join(parts))

    # Convert the modified txt content to a CSV format
    csv_lines = ['"Start";"End";"Label"']  # Start with the header
...
def plot_uniform_results(X_dict_STFT, ann_dict_STFT, X_dict_CQT, ann_dict_CQT, X_dict_IIR, ann_dict_IIR, song_selected, song_dict, chord_labels):
    for s in song_selected:
